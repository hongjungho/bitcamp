...
// 소스 코드와 컴파일로 만들어진 바이트코드파일을 분리해 두면
// 나중에 사용자에게 배포 하는 파일을 만들 때 편하다.
// 그래서 보통 다음과 같은 폴더 이름으로 분류한다.
// src - 소스 파일을 두는 폴더
// bin - binary 파일(컴파일로 생성된 바이트코드 파일)을 두는 폴더
//
// 컴파일 방법
// javac -d 컴파일결과를둘폴더 소스파일경로
// javac -d ./bin ./src/Exam05.java
//
// 다른 폴더에 있는 바이트코드 실행하기 ( 내가궁금했던것 )
// 1) 그폴더로 찾아가서 실행한다. 비추천

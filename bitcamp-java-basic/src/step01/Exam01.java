...
// 소스파일이 놓이는 루트 폴더의 하위 폴더명을 적어야한다
// 즉 src폴더는 소스파일을 두는 루트 폴더이기 때문에
// package 명에 포함 해서는 안된다
...
// => 패키지명은 보통 소스 파일이 있는 경로와 같게 한다.
// => 다르게 해도 되는데 소스파일 관리하기 힘들다.
// => -d 옵션으로 컴파일하면 패키지명에 해당하는 폴더가
//    자동으로 생성되고 그폴더에 .class 파일이 놓인다.
//
// => 패키지에 소속된 클래스 실행하기
// javac -classpath 패키지의위치 패키지명.클래스명
// javac -classpath bin step01.Exam01
// => -classpath에는 반드시 패키지가 있는 위치를 지정해야 한다 (클래스가있는곳이아니라 패키지가있는위치 step01 X bin O)
//
// 패키지 이름을짓는 규칙
